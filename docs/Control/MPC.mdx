...
![lambda](https://latex.codecogs.com/svg.image?\color%7Bwhite%7D%5Clambda) is a blending parameter defined in the "Variables and parameters" section below.
...
- ![e_c](https://latex.codecogs.com/svg.image?\color%7Bwhite%7D%5Chat%7Be%7D_c), the contouring error, the error perpendicular to the reference path
    ...
    - ![e_c eqn](https://latex.codecogs.com/svg.image?\color%7Bwhite%7D\hat%7Be%7D_c(X,Y,\theta)=\sin(\Phi(\theta))(X-X_%7B\text%7Bref%7D%7D(\theta))-\cos(\Phi(\theta))(Y-Y_%7B\text%7Bref%7D%7D(\theta)))
